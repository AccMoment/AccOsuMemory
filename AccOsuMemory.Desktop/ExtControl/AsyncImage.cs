    ...
    private async Task<IImage?> GetImageFromSource(string sourcePath, double rescaleToHeight = double.NaN)
    {
        return await Task.Run(async () =>
        {
            ...
            try
            {
                ...
                {
                    var response = await DownloadManager.GetHttpClient().GetAsync(sourcePath, HttpCompletionOption.ResponseContentRead);
                ...
                return double.IsNaN(rescaleToHeight) ? new Bitmap(imageStream) : Bitmap.DecodeToHeight(imageStream, (int)rescaleToHeight, InterpolationMode);
            }
            catch (Exception)
            {
